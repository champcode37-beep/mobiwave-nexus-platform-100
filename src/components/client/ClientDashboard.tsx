...
export function ClientDashboard() {
  ...
  const { isConnected, latestUpdate } = useRealTimeUpdates({
    userId: isClientProfile ? clientProfile?.id : user?.id,
        ...
        <div className="space-y-6 p-3 sm:p-4 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600">
                Welcome back{isClientProfile && clientProfile ? `, ${clientProfile.client_name}` : ''}! Here's what's happening with your account.
              </p>
            </div>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              { title: 'SMS Credits', value: `${remainingCredits.toFixed(0)} SMS`, icon: MessageSquare, path: '/billing' }
            ].map((metric, index) => (
              <Card key={index}>
