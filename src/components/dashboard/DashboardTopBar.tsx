...
export function DashboardTopBar() {
  const { user, clientProfile, isClientProfile, logout } = useAuth();
  const location = useLocation();
  const navigate = useNavigate();

  const isAdminRoute = location.pathname.startsWith('/admin');

  // Fetch user profile to get first_name and last_name for regular users
  const { data: userProfile } = useQuery({
    queryKey: ['user-profile', user?.id],
    queryFn: async () => {
      if (!user?.id || isClientProfile) return null;
    ...
    enabled: !!user?.id && !isClientProfile
  });
  ...
  const getUserDisplayName = () => {
    // For client profiles, extract first name from client_name
    if (isClientProfile && clientProfile?.client_name) {
      return clientProfile.client_name.split(' ')[0];
    }

    // For regular users, use profile first name
